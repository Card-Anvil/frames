...
`packages/frame-studio` is a React + Chakra UI v3 app, built into
`frame-kit/dist/studio` and served by `frame-kit studio`. It is the only UI in
this repo, and the conventions are the app's:

- **Chakra v3 only.** v2 patterns do not apply — no `extendTheme`, no
  `useColorModeValue`, no `sx`, no `colorScheme`. The `chakra-ui-builder` and
  `chakra-ui-refactor` skills and the `chakra-ui` MCP server are the sources of
  truth.
- **Semantic tokens, never hex.** `bg`, `bg.panel`, `bg.muted`, `fg`,
  `fg.muted`, `border`, `fg.error`. A raw hex bypasses the theme and breaks one
  of the two colour modes — both of which the studio ships.
- **Components come from the CLI.** `npx @chakra-ui/cli snippet add <name>`
  writes into `src/components/ui/`. Those files are generated: do not hand-edit
  them, and do not hold them to this repo's lint style — `eslint.config.js`
  exempts them for exactly that reason. The CLI installs their dependencies
  into a nested `node_modules` under `src/`; move those to the package's own
  `devDependencies` and delete the nested copy, because a `package.json` inside
  `src` decides CJS/ESM for that subtree under `nodenext`.

Two things genuinely cannot use tokens, and both are commented where they
appear: Konva paints to a canvas and cannot read the CSS variables tokens
compile to, and the WUBRG swatches are Magic's colours rather than the theme's.

## Agent configuration

Configuration is written once and fanned out, so every agent sees the same
servers and skills.
...
**Generated by `pnpm sync:agents` — do not edit:** `.cursor/mcp.json`,
`.vscode/mcp.json`, `.codex/config.toml`, and `.claude/skills/` (gitignored,
because Claude Code only reads that path).

After editing `.mcp.json` or `.agents/skills/`, run `pnpm sync:agents` and
commit the result. `pnpm sync:agents:check` fails if they have drifted.

| Server                | Use it for                               |
| --------------------- | ---------------------------------------- |
| `chakra-ui`           | Chakra v3 component APIs, props, theming |
| `konva-documentation` | Konva canvas APIs, used by the studio    |
